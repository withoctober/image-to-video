...
export function Select({ size, status, className, value, onChange, placeholder, options }: SelectProps) {
  ...
  return (
    <SelectPrimitive selectedOption={selectedOption} onChange={(option) => onChange?.(option?.value)}>
      {({ selectedOption }) => (
        <>
          <SelectTrigger className={`${selectClasses({ size, status, className })}`}>
            <span className="flex max-w-full items-center justify-between">
              <span className="block flex-1 truncate">{selectedOption?.label ?? placeholder ?? 'Select option'}</span>
