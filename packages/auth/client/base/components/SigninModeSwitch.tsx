...
export default function SigninModeSwitch({
  ...
  return (
    <div
      className={`${className} flex w-full justify-center rounded-full bg-zinc-100 p-1 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300`}
    >
      {modes.map((mode) => (
        <button
          key={mode.key}
          type="button"
          className={`block w-1/2 rounded-full px-6 py-1.5 text-xs ${
            mode.key === activeMode
