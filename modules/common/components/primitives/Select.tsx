...
function Select({ size, status, className, value, onChange, placeholder, options }: SelectProps) {
  ...
  useEffect(() => {
    if (selectedOption && selectedOption !== api.selectedOption) api.setSelectedOption(selectedOption);
  }, [selectedOption, api]);

  useEffect(() => {
    if (typeof onChange !== 'undefined' && api.selectedOption?.value !== value)
      onChange(api.selectedOption?.value ?? undefined);
  }, [api.selectedOption]); // eslint-disable-line react-hooks/exhaustive-deps
