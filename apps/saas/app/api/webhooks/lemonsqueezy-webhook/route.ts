...
if (!lemonsqueezySigningSecret) throw new Error('Missing env for lemonsqueezy signing secret');
...
export async function POST(req: Request) {
  try {
    ...
    const signature = Buffer.from(headers().get('x-signature') as string, 'utf8');
