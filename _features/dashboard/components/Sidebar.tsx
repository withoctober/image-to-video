          ...
          <li key={menuItem.href}>
            <Link
              ...
              className={`flex items-center gap-3 rounded-xl px-4 py-3 hover:text-zinc-950 hover:no-underline focus:no-underline dark:hover:text-white ${
                isActiveMenuItem(menuItem.segment) ? 'bg-primary-500/10 font-bold text-zinc-950 dark:text-white' : ''
              }`}
            >
              <menuItem.icon
                className={`h-6 w-6 transform ${
                  isActiveMenuItem(menuItem.segment) ? 'text-primary-500' : 'text-zinc-950/50 dark:text-white/50'
                }`}
              />
